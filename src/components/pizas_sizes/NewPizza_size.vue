<template>
  <div class="container text-start">
    <h1 class="text-danger fw-bold">{{ $t("pizza_sizes.newTitle") }}</h1>
    <div class="card">
...
export default {
  ...
  methods: {
    ...
    async createPizzaSize() {
      this.loading = true;
      try {
        await axios.post("/api/pizza-sizes", this.pizzaSize);
        Swal.fire({
          position: "top-end",
          icon: "success",
          title: this.$t("pizza_sizes.createdSuccess"),
          showConfirmButton: false,
          timer: 2000,
        });
        this.$router.push({ name: "PizzaSizes" });
      } catch (error) {
        let errorMsg = this.$t("pizza_sizes.createError");
